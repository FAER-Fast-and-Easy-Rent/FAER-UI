...
export default function Header({ user }: Props) {
    ...
    return (<header className="flex flex-col bg-white sticky top-0 z-10 w-full place-content-center shadow-sm">
        <nav className="flex justify-between px-4 py-4 xl:px-0 w-full max-w-7xl mx-auto items-center">
            ...
            <div className=" flex shrink-0 items-center space-x-2 text-gray-600 hover:text-gray-800 cursor-pointer" onClick={() => setButton(!button)}>
                <span className="w-6 h-6 rounded-full bg-gradient-to-br from-sky-500 via-violet-500 to-indigo-600"></span>
                <span>{user?.name}</span>
                <span><svg className="w-6 h-6 text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg></span>
